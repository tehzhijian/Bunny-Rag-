...
class BunnyPathRetriever:
    ...
    def build_effective_resistance(self, json_path: str):
        ...
        return label_to_id, id_to_label, R
    ...
    def retrieve_nodes_part2(self, 
        ...
        try:
            label_to_id, id_to_label, R = self.build_effective_resistance(json_path)
            seed_list = self.retrieve_nodes(query)
            print(seed_list)
            scores = {}
            res = [100]*len(label_to_id)
            sim = [0]*len(label_to_id)
            for node_id, emb in self.node_embeddings.items():
                ...
                for node_sp, _ in seed_list:
                    if node_sp in label_to_id:
                        ...
                        simmi = util.pytorch_cos_sim(sp_emb, emb).item()
                        sim_i -= labda * simmi
            ...
            for i in range(len(res)):
                score_i = float(1/(res[i])-sim[i]/len(seed_list))
                node_id = id_to_label[i]    
                scores[node_id] = float(score_i)
                #print(node_id, res[i])
